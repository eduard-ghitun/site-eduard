...
const ContactMethod = ({ item, variant = "card", delay = 0 }) => {
  ...
  if (variant === "inline") {
    return (
      <a
        href={item.href}
        {...externalProps}
        className={`group inline-flex min-h-11 items-center gap-2 rounded-full border border-[rgba(121,255,172,0.12)] bg-[rgba(8,13,11,0.72)] px-3 py-1.5 text-xs text-[color:var(--text-soft)] transition duration-300 ${
  ...
  return (
    <ScrollReveal
      ...
      threshold={0.16}
      glow
      className={`group relative min-h-11 overflow-hidden rounded-[1.5rem] border border-[rgba(121,255,172,0.12)] bg-[rgba(8,13,11,0.58)] p-4 transition duration-300 ${
        disableHover ? "" : "hover:border-[rgba(121,255,172,0.22)] hover:bg-[rgba(9,18,14,0.72)]"
      }`}
    >
      {!disableHover ? (
        <div className="pointer-events-none absolute inset-0 opacity-0 transition duration-300 group-hover:opacity-100 [background:radial-gradient(circle_at_10%_10%,rgba(92,255,154,0.12),transparent_50%)]" />
      ) : null}
